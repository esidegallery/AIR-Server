	...
	public class NativeObjectSerializer implements IMessageSerializer
	{
		...
		public function deserialize(serialized:*):Vector.<Message>
		{
			...
			else
			{
				...
				if (serialized.hasOwnProperty("senderId"))
					message.senderId = serialized.senderId;
				if (serialized.hasOwnProperty("command"))
					message.command = serialized.command;
				if (serialized.hasOwnProperty("data"))
					message.data = serialized.data;
				else
